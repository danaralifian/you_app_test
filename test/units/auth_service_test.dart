...
void main() {
  ...
  test('login returns AuthResponse when Dio responds successfully', () async {
    debugPrint(
      'Running test: login returns AuthResponse when Dio responds successfully',
    );
  ...
  test('register calls /register then login and returns AuthResponse', () async {
    debugPrint(
      'Running test: register calls /register then login and returns AuthResponse',
    );
    final request = AuthRequest(
      email: 'test@example.com',
      username: 'testuser',
    ...
    verify(() => mockDio.post('/register', data: any(named: 'data'))).called(1);
